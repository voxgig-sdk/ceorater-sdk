...
module CeoraterUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = CeoraterResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CeoraterResponse.new(fetched)
    else
