...
class CompanyEntity
  ...
  def make
    opts = @_entopts.dup
    CompanyEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CeoraterHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Company, Hash] the current Company data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CeoraterHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Company fields)
  def match_get
  ...
  #
  # @param reqmatch [CompanyLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Company.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Company, Hash] the loaded Company; raises CeoraterError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = CeoraterHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [CompanyListMatch, Hash, nil] match filter (any subset of
  #   Company fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Company>, Array] the matching Company items; raises CeoraterError on failure
