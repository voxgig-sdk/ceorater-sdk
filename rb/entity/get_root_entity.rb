...
class GetRootEntity
  ...
  def make
    opts = @_entopts.dup
    GetRootEntity.new(@_client, opts)
  end
  ...
  # @return [GetRoot, Hash] the current GetRoot data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of GetRoot fields)
  def match_get
  ...
  #
  # @param reqmatch [GetRootLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.GetRoot.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [GetRoot, Hash] the loaded GetRoot; raises CeoraterError on failure
