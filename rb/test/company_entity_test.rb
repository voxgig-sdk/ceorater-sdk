...
class CompanyEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CEORATER_TEST_COMPANY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    company_ref01_data = nil
    if company_ref01_data_raw.length > 0
...
def company_basic_setup(extra)
  ...
  entid_env_raw = ENV["CEORATER_TEST_COMPANY_ENTID"]
