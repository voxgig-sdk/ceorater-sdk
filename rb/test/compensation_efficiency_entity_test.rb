...
class CompensationEfficiencyEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.CompensationEfficiency(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.CompensationEfficiency(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    compensation_efficiency_ref01_list_result = compensation_efficiency_ref01_ent.list(compensation_efficiency_ref01_match, nil)
