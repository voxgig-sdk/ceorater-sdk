...
[MCP](https://modelcontextprotocol.io) server exposing the Ceorater SDK as
two agent tools — `ceorater_list` and `ceorater_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ceorater-mcp)
make build

# 2. Provide credentials via the environment
export CEORATER_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user ceorater \
  -- /absolute/path/to/ceorater-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./ceorater-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// ceorater_list: first page of records
{ "entity": "ceo_performance" }
{ "entity": "ceo_performance", "query": { } }

// ceorater_load: one record by id
{ "entity": "company", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ceorater-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user ceorater \
     -- "$PWD"/dist/*/ceorater-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `ceorater_list` and `ceorater_load` tools now appear
   in new sessions. Ask the agent to *"list ceo_performance using ceorater"*
   and it calls `ceorater_list` with `{"entity":"ceo_performance"}`.
...
```sh
export CEORATER_APIKEY=sk_live_xxx            # API key
export CEORATER_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `ceorater_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `ceorater_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CEORATER_APIKEY` | API key sent with every request. |
| `CEORATER_BASE` | Optional override of the API base URL. |

### Entities

The 6 entities valid as the `entity` argument:

ceo_performance | company | compensation_efficiency | general | get_root | search

### Smoke test via HTTP (raw JSON-RPC)

```sh
./ceorater-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ceorater_load","arguments":{"entity":"company","query":{"id":1}}}}'
```
