...
Create an instance: `local ceo_performance = client:CeoPerformance(nil)`
...
```lua
local ceo_performances, err = client:CeoPerformance():list()
```
...
Create an instance: `local compensation_efficiency = client:CompensationEfficiency(nil)`
...
```lua
local compensation_efficiencys, err = client:CompensationEfficiency():list()
```
...
```
lua/
├── ceorater_sdk.lua    -- Main SDK module
...
The main module (`ceorater_sdk`) exports the SDK constructor
