...
The Lua SDK for the Ceorater API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:CeoPerformance(nil):list(nil, nil)
...
local result, err = client:Ceorater(nil):load(
  { id = "test01" }, nil
)
