...
local CompanyEntity = {}
CompanyEntity.__index = CompanyEntity


function CompanyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CompanyEntity)
  self._name = "company"
  self._client = client
...
function CompanyEntity:get_name()
  return self._name
end


function CompanyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CompanyEntity.new(self._client, opts)
end
...
function CompanyEntity:mark_deleted()
  self._deleted = true
end


function CompanyEntity:deleted()
  return true == self._deleted
end


function CompanyEntity:data_set(args)
  if args ~= nil then
...
function CompanyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CompanyEntity:match_set(args)
  if args ~= nil then
...
function CompanyEntity:match_get()
...
function CompanyEntity:stream(action, args, callopts)
...
---@param reqmatch CompanyLoadMatch
---@param ctrl? table
---@return Company
---@return string? err
function CompanyEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch CompanyListMatch
---@param ctrl? table
---@return Company[]
---@return string? err
function CompanyEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function CompanyEntity:_run_op(ctx, post_done)
  local utility = self._utility
