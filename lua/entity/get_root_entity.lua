...
local GetRootEntity = {}
GetRootEntity.__index = GetRootEntity


function GetRootEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GetRootEntity)
  self._name = "get_root"
  self._client = client
...
function GetRootEntity:get_name()
  return self._name
end


function GetRootEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GetRootEntity.new(self._client, opts)
end
...
function GetRootEntity:mark_deleted()
  self._deleted = true
end


function GetRootEntity:deleted()
  return true == self._deleted
end


function GetRootEntity:data_set(args)
  if args ~= nil then
...
function GetRootEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GetRootEntity:match_set(args)
  if args ~= nil then
...
function GetRootEntity:match_get()
...
function GetRootEntity:stream(action, args, callopts)
...
function GetRootEntity:_run_op(ctx, post_done)
  local utility = self._utility
