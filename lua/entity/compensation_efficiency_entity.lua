-- Ceorater SDK CompensationEfficiency entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local CompensationEfficiencyEntity = {}
CompensationEfficiencyEntity.__index = CompensationEfficiencyEntity


function CompensationEfficiencyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CompensationEfficiencyEntity)
  self._name = "compensation_efficiency"
  self._client = client
...
function CompensationEfficiencyEntity:get_name()
  return self._name
end


function CompensationEfficiencyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CompensationEfficiencyEntity.new(self._client, opts)
end
...
function CompensationEfficiencyEntity:mark_deleted()
  self._deleted = true
end


function CompensationEfficiencyEntity:deleted()
  return true == self._deleted
end


function CompensationEfficiencyEntity:data_set(args)
  if args ~= nil then
...
function CompensationEfficiencyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CompensationEfficiencyEntity:match_set(args)
  if args ~= nil then
...
function CompensationEfficiencyEntity:match_get()
...
function CompensationEfficiencyEntity:stream(action, args, callopts)
...
---@param reqmatch CompensationEfficiencyListMatch
---@param ctrl? table
---@return CompensationEfficiency[]
---@return string? err
function CompensationEfficiencyEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function CompensationEfficiencyEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return CompensationEfficiencyEntity
