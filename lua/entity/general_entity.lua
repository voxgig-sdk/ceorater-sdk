...
local GeneralEntity = {}
GeneralEntity.__index = GeneralEntity


function GeneralEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GeneralEntity)
  self._name = "general"
  self._client = client
...
function GeneralEntity:get_name()
  return self._name
end


function GeneralEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GeneralEntity.new(self._client, opts)
end
...
function GeneralEntity:mark_deleted()
  self._deleted = true
end


function GeneralEntity:deleted()
  return true == self._deleted
end


function GeneralEntity:data_set(args)
  if args ~= nil then
...
function GeneralEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GeneralEntity:match_set(args)
  if args ~= nil then
...
function GeneralEntity:match_get()
...
function GeneralEntity:stream(action, args, callopts)
...
function GeneralEntity:_run_op(ctx, post_done)
  local utility = self._utility
