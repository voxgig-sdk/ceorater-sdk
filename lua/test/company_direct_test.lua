...
describe("CompanyDirect", function()
  it("should direct-list-company", function()
    local setup = company_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-company", setup.live and "live" or "unit")
  ...
  it("should direct-load-company", function()
    local setup = company_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-company", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function company_direct_setup(mockres)
