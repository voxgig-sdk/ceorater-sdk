...
The CEORater API provides programmatic access to CEO performance metrics, compensation efficiency, and detailed company data.

Learn more about CEORater API at [ceorater-api.onrender.com](https://ceorater-api.onrender.com).
...
This SDK exposes the API as a small set of **semantic entities** — CeoPerformance, Company, CompensationEfficiency, General, GetRoot and Search — that you
...
const items = await client.CeoPerformance().list()
```
...
$ceoperformances = $client->CeoPerformance()->list();
print_r(array_map(fn($item) => $item->data_get(), $ceoperformances));
```
