...
ceoperformance, err := client.CeoPerformance(nil).Load(
    map[string]any{"id": "test01"}, nil,
)
if err != nil {
    panic(err)
}
fmt.Println(ceoperformance) // the loaded mock data
```
...
slice):
    ...
    ceoperformance, err := client.CeoPerformance(nil).Load(map[string]any{"id": "example_id"}, nil)
    if err != nil { /* handle */ }
    // ceoperformance is the loaded record
...
```go
general, err := client.General(nil).Load(map[string]any{"id": "general_id"}, nil)
if err != nil {
    panic(err)
}
...
```go
get_root, err := client.GetRoot(nil).Load(map[string]any{"id": "get_root_id"}, nil)
if err != nil {
    panic(err)
}
...
ceoperformance.Load(map[string]any{"id": "example_id"}, nil)
