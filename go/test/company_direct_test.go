...
func TestCompanyDirect(t *testing.T) {
	t.Run("direct-list-company", func(t *testing.T) {
		setup := companyDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-company", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-company", func(t *testing.T) {
		setup := companyDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-company", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type companyDirectSetupResult struct {
...
func companyDirectSetup(mockres any) *companyDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CEORATER_TEST_COMPANY_ENTID"]; ok {
		...
		return &companyDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &companyDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
