...
func TestCompensationEfficiencyEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.CompensationEfficiency(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.CompensationEfficiency(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		compensationEfficiencyRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.compensation_efficiency")))
