...
import (
	...
	omni "github.com/voxgig-sdk/ceorater-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CeoraterError); is {
				out := map[string]any{
					"name":    "CeoraterError",
...
func omniCtx(arg any, client *sdk.CeoraterSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CeoraterSDK); is {
			client = live
