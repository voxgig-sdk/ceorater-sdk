...
class FtCtrl extends CeoraterControl
{
...
class FtClient
{
    ...
    private CeoraterUtility $utility;

    public function __construct(CeoraterUtility $utility, array $options)
    {
    ...
    public function get_utility(): CeoraterUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CeoraterContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CeoraterContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CeoraterContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CeoraterConfig::make_feature($name);
    ...
    private static function buildUrl(CeoraterSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CeoraterError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CeoraterResponse($fetched);
        }
    ...
    private function fail(CeoraterContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CeoraterContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
