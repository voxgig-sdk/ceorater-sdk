...
class CeoraterSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = CeoraterHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = CeoraterHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!CeoraterFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, CeoraterFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return CeoraterUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = CeoraterHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = CeoraterHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = CeoraterHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new CeoraterError($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = CeoraterHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new CeoraterError("graphql_error",
                "CeoraterSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
