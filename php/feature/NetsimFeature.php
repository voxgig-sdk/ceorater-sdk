...
class CeoraterNetsimFeature extends CeoraterBaseFeature
{
    ...
    private function _simulate(CeoraterContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(CeoraterContext $ctx, array $applied): void
    {
        $client = $this->client;
