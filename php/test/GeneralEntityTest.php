...
class GeneralEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CeoraterSDK::test(null, null);
        $ent = $testsdk->General(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = general_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "general." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CEORATER_TEST_GENERAL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $general_ref01_data = null;
        if (count($general_ref01_data_raw) > 0) {
...
function general_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/general/GeneralTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["general01", "general02", "general03"] as $k) {
    ...
    $entid_env_raw = getenv("CEORATER_TEST_GENERAL_ENTID");
