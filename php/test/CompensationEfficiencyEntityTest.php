...
class CompensationEfficiencyEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CeoraterSDK::test(null, null);
        $ent = $testsdk->CompensationEfficiency(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->CompensationEfficiency(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->CompensationEfficiency(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = compensation_efficiency_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "compensation_efficiency." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CEORATER_TEST_COMPENSATION_EFFICIENCY_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $compensation_efficiency_ref01_data = null;
        if (count($compensation_efficiency_ref01_data_raw) > 0) {
...
function compensation_efficiency_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/compensation_efficiency/CompensationEfficiencyTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["compensation_efficiency01", "compensation_efficiency02", "compensation_efficiency03"] as $k) {
    ...
    $entid_env_raw = getenv("CEORATER_TEST_COMPENSATION_EFFICIENCY_ENTID");
