...
boru-driven command-line client **and** interactive REPL for the Ceorater
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ceorater-cli)
make build
...
./ceorater-cli list ceo_performance
./ceorater-cli list company

# 5. Override the API base URL for a single call
CEORATER_BASE=https://api.example.com ./ceorater-cli list ceo_performance

# 6. No arguments -> interactive REPL
./ceorater-cli
ceorater> list ceo_performance
ceorater> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ceorater-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/ceorater-cli list ceo_performance
   ```
...
Run with no arguments to open a REPL (prompt `ceorater>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `ceo_performance`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ceorater-cli`. |
...
(`./ceorater-cli <expr>`) and interactively (the REPL), and expressions compose the
