// Command Ceorater-cli is a CLI + REPL for the Ceorater SDK,
...
const prompt = "ceorater"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "ceo_performance company compensation_efficiency general get_root search"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CEORATER_APIKEY (api key), CEORATER_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CEORATER_APIKEY carries the API key and
	// CEORATER_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CEORATER_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CEORATER_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCeoraterSDK(opts)
