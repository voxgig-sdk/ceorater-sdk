...
The Python SDK for the Ceorater API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.CeoPerformance(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.Ceorater(None).load(
    {"id": "test01"}, None
)
