...
class GetRootEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return GetRootEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> GetRoot:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.GetRoot().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
