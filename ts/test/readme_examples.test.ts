...
//     (\`new CeoraterSDK(...)\` / \`CeoraterSDK.test(...)\` become
//     \`CeoraterSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"ceo_performance":{"test01":{"id":"test01"}},"company":{"test01":{"id":"test01"}},"compensation_efficiency":{"test01":{"id":"test01"}},"general":{"test01":{"id":"test01"}},"get_root":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CeoraterSDK(...)\` and any
// \`CeoraterSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CeoraterSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CeoraterSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
