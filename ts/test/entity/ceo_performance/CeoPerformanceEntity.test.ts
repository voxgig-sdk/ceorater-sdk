...
describe('CeoPerformanceEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'ceo_performance.' + op, live)) return
    }
    ...
    const ceo_performance_ref01_list = await ceo_performance_ref01_ent.list(ceo_performance_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['CEORATER_TEST_CEO_PERFORMANCE_ENTID']
