...
(0, node_test_1.describe)('CompanyDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-company', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-company', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-company', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-company', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.CEORATER_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.CeoraterSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['CEORATER_TEST_COMPANY_ENTID'];
    ...
    const client = new __1.CeoraterSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=CompanyDirect.test.js.map